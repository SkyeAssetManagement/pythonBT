...
class EquityCurveWidget(QWidget):
    ...
    def _setup_ui(self):
        ...
        # Chart area with datetime axis
        # Create PlotWidget with DateAxisItem for proper time formatting
    ...
    def _setup_chart(self):
        ...
        # Setup axes with proper time formatting
        self.plot_widget.setLabel('left', 'Portfolio Value', color='white', size='10pt')
        self.plot_widget.setLabel('bottom', 'Time', color='white', size='10pt')
    ...
    def load_equity_data(self, equity_series: np.ndarray, timestamps: np.ndarray = None) -> bool:
        ...
        Args:
            ...
            timestamps: Optional timestamp array (uses indices if None) 
                       Can be Unix timestamps, pandas datetime, or integer nanoseconds
        """
        try:
            ...
            if timestamps is not None:
                # Convert timestamps to Unix timestamp format for DateAxisItem
                timestamps_array = np.asarray(timestamps)
                
                # Handle different timestamp formats
                if timestamps_array.dtype.kind == 'M':  # numpy datetime64
                    # Convert numpy datetime64 to Unix timestamp
                    self.timestamps = timestamps_array.astype('datetime64[s]').astype(np.float64)
                    self.use_datetime_axis = True
                    print(f"Using numpy datetime64 timestamps")
                    
                elif hasattr(timestamps, 'values') and hasattr(timestamps.values[0], 'timestamp'):  # pandas datetime
                    # Convert pandas datetime to Unix timestamp
                    self.timestamps = np.array([dt.timestamp() for dt in timestamps.values])
                    self.use_datetime_axis = True
                    print(f"Using pandas datetime timestamps")
                    
                elif timestamps_array[0] > 1e15:  # Likely nanosecond timestamps
                    # Convert nanosecond timestamps to Unix timestamp (seconds)
                    self.timestamps = timestamps_array.astype(np.float64) / 1e9
                    self.use_datetime_axis = True
                    print(f"Using nanosecond timestamps converted to Unix time")
                    
                elif timestamps_array[0] > 1e9:  # Likely Unix timestamps already
                    self.timestamps = timestamps_array.astype(np.float64)
                    self.use_datetime_axis = True
                    print(f"Using Unix timestamp format")
                    
                else:  # Use as indices
                    self.timestamps = timestamps_array.astype(np.float64)
                    self.use_datetime_axis = False
                    print(f"Using timestamps as bar indices")
            else:
                # Default to bar indices
                self.timestamps = np.arange(len(equity_series), dtype=np.float64)
                self.use_datetime_axis = False
                print(f"Using default bar indices")
            ...
            print(f"SUCCESS: Equity curve loaded - {len(self.equity_data)} points, datetime_axis: {self.use_datetime_axis}")
            return True
    ...
    def sync_viewport(self, start_time: int, end_time: int):
        ...
        Args:
            start_time: Start timestamp/index (will be converted appropriately)
            end_time: End timestamp/index (will be converted appropriately)
        """
        try:
            self.current_viewport = (start_time, end_time)
            
            # Convert start/end times to match our timestamp format
            if self.use_datetime_axis and self.timestamps is not None:
                # If we received bar indices but we're using datetime axis,
                # convert indices to timestamps
                if start_time < 1e9:  # Likely bar indices
                    if start_time < len(self.timestamps):
                        start_timestamp = self.timestamps[int(start_time)]
                    else:
                        start_timestamp = self.timestamps[-1]
                        
                    if end_time < len(self.timestamps):
                        end_timestamp = self.timestamps[int(end_time)]
                    else:
                        end_timestamp = self.timestamps[-1]
                else:
                    # Already timestamps, but may need conversion to Unix time
                    start_timestamp = start_time / 1e9 if start_time > 1e15 else start_time
                    end_timestamp = end_time / 1e9 if end_time > 1e15 else end_time
                    
                # Update chart viewport with timestamp values
                padding = (end_timestamp - start_timestamp) * 0.05  # 5% padding
                self.plot_widget.setXRange(
                    start_timestamp - padding,
                    end_timestamp + padding,
                    padding=0
                )
                
                print(f"Equity curve viewport synced with datetime: {datetime.fromtimestamp(start_timestamp)} - {datetime.fromtimestamp(end_timestamp)}")
                ...
                print(f"Equity curve viewport synced with indices: {start_time} - {end_time}")
            
            # Auto-range Y axis for visible data
            if self.equity_data is not None and self.timestamps is not None:
                if self.use_datetime_axis:
                    # Find data range using timestamps
                    start_search = start_timestamp if 'start_timestamp' in locals() else start_time
                    end_search = end_timestamp if 'end_timestamp' in locals() else end_time
                    start_idx = np.searchsorted(self.timestamps, start_search)
                    end_idx = np.searchsorted(self.timestamps, end_search)
                else:
                    # Using indices directly
                    start_idx = max(0, int(start_time))
                    end_idx = min(len(self.equity_data), int(end_time))
...
if __name__ == "__main__":
    ...
    if success:
        ...
        print("="*50)
        print("[OK] High-performance equity curve visualization")
        print("[OK] Synchronized drawdown overlay") 
        print("[OK] Trade markers with entry/exit indication")
        print("[OK] Real-time performance statistics")
        print("[OK] Interactive navigation and viewport sync")
        print("[OK] Professional trading dashboard styling")
