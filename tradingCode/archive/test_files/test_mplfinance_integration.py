...
def test_main_integration():
    ...
    try:
        ...
        print("[OK] Successfully imported main trading components")
        ...
        if data_dir.exists():
            ...
            if parquet_files:
                ...
                print(f"[OK] Loaded {len(data):,} bars in {load_time:.2f}s")
    ...
    try:
        ...
        for position, size, description in test_scenarios:
            ...
            if performance['success']:
                ...
                print(f"  [OK] SUCCESS: {metadata['final_size']:,} bars in {render_time:.2f}s")
                ...
                print(f"  [X] FAILED: {performance.get('error', 'Unknown error')}")
                all_success = False
...
def test_hybrid_dashboard_integration():
    ...
    try:
        ...
        if dashboard.initialize_qt_app():
            ...
            print("[X] Qt application initialization failed")
            return False
        
        # Test window creation
        dashboard.create_main_window()
        print("[OK] Hybrid dashboard window created")
        ...
        try:
            ...
            if success:
                print("[OK] Data loaded into hybrid dashboard")
            else:
                print("[X] Failed to load data into hybrid dashboard")
                return False
        finally:
            loop.close()
        ...
        dashboard.show()
        print("[OK] Dashboard displayed successfully")
...
def main():
    ...
    for test_name, test_func in tests:
        ...
        try:
            ...
            if success:
                print(f"[OK] PASSED ({test_time:.2f}s)")
            else:
                print(f"[X] FAILED ({test_time:.2f}s)")
    ...
    for result in results:
        status = "[OK] PASS" if result['success'] else "[X] FAIL"
