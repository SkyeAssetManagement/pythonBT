"""
Step 10: Basic zoom functionality
Goal: Implement Ctrl+scroll wheel zooming for the candlestick chart
"""
...
class ZoomableViewBox(pg.ViewBox):
    """Custom ViewBox with zoom functionality"""
    
    def __init__(self, *args, **kwargs):
        super().__init__(*args, **kwargs)
        self.setMouseMode(self.RectMode)  # Default to zoom mode
        
    def wheelEvent(self, ev, axis=None):
        """Custom wheel behavior: Ctrl+scroll zooms, regular scroll does nothing for now"""
        if ev.delta() is None:
            return
        ...
        if ctrl_pressed:
            ...
            zoom_factor = 1.2 if ev.delta() > 0 else 1/1.2
            ...
            # Calculate zoom center (mouse position or center of view)
            ...
            self.setRange(xRange=new_x_range, yRange=new_y_range, padding=0)
            
            print(f"Zoomed: X={new_x_range[0]:.1f}-{new_x_range[1]:.1f}, Y=${new_y_range[0]:.1f}-${new_y_range[1]:.1f}")
            
        else:
            # Regular scroll: Do nothing for now (will add panning in next step)
...
class ZoomableChartWidget(pg.PlotWidget):
    def __init__(self):
        ...
        self.setLabel('top', 'Zoomable Chart (Ctrl+Scroll) - Step 10', color='black', size='16pt')
        ...
        # Replace default ViewBox with zoomable one
        zoomable_vb = ZoomableViewBox()
        self.setCentralItem(zoomable_vb)
        self.plotItem.vb = zoomable_vb
        ...
        print("Step 10: Zoomable chart widget created")
    ...
    def add_zoom_instructions(self):
        ...
        instructions = pg.TextItem(
            text="Zoom Instructions: Hold Ctrl + Scroll Wheel to Zoom In/Out",
            color='blue',
...
def test_step10():
    """Test Step 10: Basic Zoom Functionality"""
    ...
    win.setWindowTitle("Step 10: Basic Zoom Functionality")
    win.resize(1300, 800)
    ...
    # Save screenshot
    save_widget_screenshot(win, "step10_zoom_functionality_result.png")
    
    print("\n=== STEP 10 VERIFICATION ===")
    print("Required elements to verify in screenshot:")
    print("- All previous elements (white background, visible axes, candlesticks)")
    print("- 150 candlesticks showing clear patterns for zoom testing")
    print("- Blue instruction text: 'Hold Ctrl + Scroll Wheel to Zoom'")
    print("- Chart initially shows all data (full zoom out)")
    print("- Ready for interactive zoom testing")
    print("\nTo test zoom functionality:")
    print("1. Hold Ctrl key")
    print("2. Scroll wheel up to zoom in")
    print("3. Scroll wheel down to zoom out")
    print("4. Observe that both X and Y axes zoom together")
    print("\nCheck step10_zoom_functionality_result.png for verification")
