...
class DataPreprocessor:
    def __init__(self, config_path='OMtree_config.ini'):
        ...
        # NEW: IQR weighting parameters
        self.iqr_weighting_enabled = self.config['preprocessing'].getboolean('iqr_weighting_enabled', fallback=False)
        self.iqr_decay_factor = float(self.config['preprocessing'].get('iqr_decay_factor', 0.98))  # Exponential decay
        
        # NEW: Winsorization parameters
        self.winsorize_enabled = self.config['preprocessing'].getboolean('winsorize_enabled', fallback=False)
        self.winsorize_percentile = float(self.config['preprocessing'].get('winsorize_percentile', 5.0))  # e.g., 5% means clip at 5th and 95th percentiles
    ...
    def weighted_iqr(self, window_data, decay_factor=0.98):
        """
        Calculate weighted IQR where more recent observations have higher weight.
        
        Args:
            window_data: Array of values in the window
            decay_factor: Exponential decay factor for weights (0 < decay < 1)
        ...
        # Create exponential weights (most recent has weight 1)
        weights = decay_factor ** np.arange(n-1, -1, -1)
        weights = weights / weights.sum()  # Normalize to sum to 1
        
        # Sort data and accumulate weights
        sorted_indices = np.argsort(window_data)
        sorted_data = window_data[sorted_indices]
        sorted_weights = weights[sorted_indices]
        cumulative_weights = np.cumsum(sorted_weights)
        
        # Find weighted percentiles
        q25_idx = np.searchsorted(cumulative_weights, self.percentile_lower / 100)
        q75_idx = np.searchsorted(cumulative_weights, self.percentile_upper / 100)
        
        # Ensure valid indices
        q25_idx = min(q25_idx, n - 1)
        q75_idx = min(q75_idx, n - 1)
        
        # Linear interpolation for exact percentile values
        if q25_idx > 0:
            w_before = cumulative_weights[q25_idx - 1] if q25_idx > 0 else 0
            w_after = cumulative_weights[q25_idx]
            target_w = self.percentile_lower / 100
            alpha = (target_w - w_before) / (w_after - w_before) if w_after > w_before else 0
            q25 = sorted_data[q25_idx - 1] * (1 - alpha) + sorted_data[q25_idx] * alpha if q25_idx > 0 else sorted_data[q25_idx]
        else:
            q25 = sorted_data[0]
        
        if q75_idx > 0:
            w_before = cumulative_weights[q75_idx - 1] if q75_idx > 0 else 0
            w_after = cumulative_weights[q75_idx]
            target_w = self.percentile_upper / 100
            alpha = (target_w - w_before) / (w_after - w_before) if w_after > w_before else 0
            q75 = sorted_data[q75_idx - 1] * (1 - alpha) + sorted_data[q75_idx] * alpha if q75_idx > 0 else sorted_data[q75_idx]
        else:
            q75 = sorted_data[0]
                ...
                for i in range(len(data)):
                    if i < self.vol_window:
                        ...
                        if len(window_data) < self.vol_window * 0.8:
                            ...
                            # Calculate IQR (weighted or standard)
                            if self.iqr_weighting_enabled:
                                iqr = self.weighted_iqr(window_data, self.iqr_decay_factor)
                            else:
    ...
    def process_data(self, df):
        ...
        if self.normalization_method == 'IQR':
            print(f"  IQR weighting: {self.iqr_weighting_enabled}")
            if self.iqr_weighting_enabled:
                print(f"  IQR decay factor: {self.iqr_decay_factor}")
        ...
        if self.normalize_target:
            ...
            processed = self.volatility_adjust(processed, [target_column], skip_normalization=False)
            target_column = f'{target_column}_vol_adj'
        else:
            print("Skipping target normalization...")
            processed = self.volatility_adjust(processed, [target_column], skip_normalization=True)
            target_column = f'{target_column}_vol_adj'
