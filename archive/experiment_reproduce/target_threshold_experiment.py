...
print("="*80)
print("TARGET_THRESHOLD OPTIMIZATION EXPERIMENT")
print("Testing values: 0.0 to 0.3 in 6 steps")
print("="*80)
...
# Test values: 0.0 to 0.3 in 6 steps
target_values = [0.0, 0.06, 0.12, 0.18, 0.24, 0.3]
results = []

for i, value in enumerate(target_values, 1):
    print(f"\n[{i}/{len(target_values)}] Testing target_threshold = {value}")
    
    # Update config
    config = configparser.ConfigParser(inline_comment_prefixes='#')
    config.read('OMtree_config.ini')
    config['model']['target_threshold'] = str(value)
    with open('OMtree_config.ini', 'w') as f:
        config.write(f)
...
df_results.to_csv('target_threshold_results.csv', index=False)

print("\n" + "="*80)
print("RESULTS SUMMARY")
print("="*80)

print("\nTarget   | Sharpe | Total PnL | Hit Rate | Edge  | Trades")
print("-"*60)
for _, row in df_results.iterrows():
    print(f"{row['target_threshold']:8.2f} | {row['sharpe_ratio']:6.3f} | "
...
if df_results['sharpe_ratio'].max() > 0:
    ...
    print(f"\nBEST TARGET_THRESHOLD:")
    print(f"  Value: {best['target_threshold']:.2f}")
    print(f"  Sharpe Ratio: {best['sharpe_ratio']:.3f}")
    print(f"  Total PnL: {best['total_pnl']:.1f}")
    print(f"  Edge: {best['edge']:.1%}")
    print(f"  Trades: {best['total_trades']:.0f}")

print("\nExperiment complete! Results saved to target_threshold_results.csv")