...
class RegressionAnalysisTab:
    def __init__(self, parent_notebook):
        ...
        self.use_normalized_data = True  # Default to normalized data
    ...
    def setup_regression_tab(self):
        ...
        # === Data Type Selection ===
        data_type_frame = ttk.LabelFrame(left_panel, text="Data Type", padding=5)
        data_type_frame.pack(fill='x', pady=(0, 10))
        
        self.data_type_var = tk.StringVar(value="normalized")
        ttk.Radiobutton(data_type_frame, text="Volatility Normalized", 
                       variable=self.data_type_var, value="normalized",
                       command=self.update_data_type).pack(anchor='w')
        ttk.Radiobutton(data_type_frame, text="Raw Data", 
                       variable=self.data_type_var, value="raw",
                       command=self.update_data_type).pack(anchor='w')
        
        self.data_type_info = ttk.Label(data_type_frame, text="Using: Volatility Normalized Data", 
                                       foreground='green', font=('Arial', 9, 'italic'))
        self.data_type_info.pack(anchor='w', pady=(5, 0))
    ...
    def update_data_type(self):
        """Update data type selection info"""
        if self.data_type_var.get() == "normalized":
            self.use_normalized_data = True
            self.data_type_info.config(text="Using: Volatility Normalized Data")
        else:
            self.use_normalized_data = False
            self.data_type_info.config(text="Using: Raw Data")
    ...
    def apply_target_filter(self, df, target_columns):
        ...
        data_type_str = "NORM" if self.use_normalized_data else "RAW"
        if filter_type == "UP":
            filter_desc = f"target > {threshold}"
        else:
            filter_desc = f"target < -{threshold}"
        
        self.status_var.set(f"Filter {filter_type} ({filter_desc}, {data_type_str}): {filtered_count}/{original_count} rows ({percent_retained:.1f}%)")
    ...
    def update_variable_lists(self):
        ...
        # Determine which columns to show based on data type selection
        if self.use_normalized_data and len(self.feature_columns_vol) > 0:
    ...
    def display_results(self, results_matrix, p_value_matrix, summary_stats):
        ...
        # Get filter and data type information for titles
        data_type_str = "NORM" if self.use_normalized_data else "RAW"
        filter_info = f" [{data_type_str}, {self.filter_type.get()} filter"
        if self.filter_type.get() != "ALL":
