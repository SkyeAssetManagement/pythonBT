...
Calculates advanced trading metrics including Sharpe ratio, drawdown, etc.
"""
...
def calculate_performance_stats(df, annual_trading_days=252, model_type='longonly'):
    ...
    if 'target_value' in trades.columns:
        if model_type == 'shortonly':
            # Shorts profit from negative returns
        ...
        stats['total_return'] = trades['pnl'].sum()
        stats['average_return'] = trades['pnl'].mean()
        stats['return_std'] = trades['pnl'].std()
            ...
            try:
                ...
                if years > 0:
                    ...
                    if stats['return_std'] > 0:
                        ...
                        annual_return = stats['total_return'] / years
                        stats['sharpe_ratio'] = annual_return / annualized_std if annualized_std > 0 else 0
                    else:
                    ...
                    if stats['max_drawdown'] != 0:
                        stats['calmar_ratio'] = (stats['total_return'] / years) / abs(stats['max_drawdown'])
                    else:
                        stats['calmar_ratio'] = np.inf if stats['total_return'] > 0 else 0
                    
                    # Monthly statistics
                    monthly_returns = trades.set_index('date')['pnl'].resample('ME').sum()
                    stats['best_month'] = monthly_returns.max()
                    stats['worst_month'] = monthly_returns.min()
...
def format_stats_for_display(stats):
    ...
    if 'total_return' in stats:
        output.append(f"Total Return: {stats['total_return']:.4f}")
    ...
    if 'max_drawdown' in stats:
        output.append(f"Max Drawdown: {stats['max_drawdown']:.4f}")
    ...
    if 'best_month' in stats:
        output.append("\nMONTHLY PERFORMANCE")
        output.append("-" * 40)
        output.append(f"Best Month: {stats['best_month']:.4f}")
        output.append(f"Worst Month: {stats['worst_month']:.4f}")
