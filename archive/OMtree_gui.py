...
class OMtreeGUI:
    ...
    def setup_data_tab(self):
        ...
        validation_frame = ttk.LabelFrame(main_frame, text="Validation Period Settings", padding=10)
        ...
        # Timeline visualization (smaller)
        timeline_frame = ttk.Frame(validation_frame, height=150)
        timeline_frame.pack(fill='x', pady=(10, 0))
        self.timeline = TimelineVisualization(timeline_frame)
        
        # Field selection section (compact)
        field_frame = ttk.LabelFrame(main_frame, text="Field Selection", padding=5)
        ...
        self.available_listbox = tk.Listbox(avail_frame, selectmode='multiple', height=6, exportselection=False)
        ...
        ttk.Button(button_frame, text="→F→", command=self.move_to_features).pack(pady=2)
        ttk.Button(button_frame, text="←F←", command=self.remove_from_features).pack(pady=2)
        ttk.Button(button_frame, text="→T→", command=self.move_to_targets).pack(pady=2)
        ttk.Button(button_frame, text="←T←", command=self.remove_from_targets).pack(pady=2)
        
        # Feature columns
        feature_frame = ttk.LabelFrame(columns_frame, text="Features", padding=3)
        feature_frame.grid(row=0, column=2, sticky='nsew', padx=2)
        
        self.feature_listbox = tk.Listbox(feature_frame, selectmode='multiple', height=6, exportselection=False)
        ...
        self.target_listbox = tk.Listbox(target_frame, selectmode='multiple', height=6, exportselection=False)
